...
# ─────────────────────────────────────────────────────────────────────────────
# 1. app/dashboard/absensi/siswa/page.tsx
# ─────────────────────────────────────────────────────────────────────────────
...
import type { JadwalHariIniItem } from '@/types'

export default function AbsensiSiswaPage() {
  const { jadwalList, isLoading, aktiveSemesterNama } = useMyStatusHariIni()
  const [izinTarget, setIzinTarget] = useState<JadwalHariIniItem | null>(null)
...
# ─────────────────────────────────────────────────────────────────────────────
# 2. app/dashboard/absensi/siswa/_components/JadwalSiswaCard.tsx
# ─────────────────────────────────────────────────────────────────────────────
...
export function JadwalSiswaCard({ item, onIzin }: Props) {
  ...
  // Sudah ada status final — tidak perlu tombol aksi
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4 space-y-3 hover:shadow-sm transition-shadow">
      ...
      {/* Top row: nama mapel + status badge */}
      ...
      {/* Mode sesi indicator — hanya tampil saat sesi aktif */}
      ...
      {/* Tombol izin — sembunyikan jika sudah ada status final */}
      {!sudahTercatat && (
...
# ─────────────────────────────────────────────────────────────────────────────
# 3. app/dashboard/absensi/siswa/_components/PengajuanIzinModal.tsx
# ─────────────────────────────────────────────────────────────────────────────
...
import type { JadwalHariIniItem, JenisPerizinan } from '@/types'
...
const schema = z
  .object({
    jenis:          z.enum(['SAKIT', 'IZIN', 'CUTI', 'DINAS', 'KEPERLUAN_KELUARGA'], {
...
export function PengajuanIzinModal({ open, onClose, jadwal }: Props) {
  ...
  // Reset tiap kali modal dibuka ulang
  useEffect(() => {
    if (!open) return
        ...
        <div className="flex flex-col items-center justify-center py-10 gap-3">
          <div className="w-14 h-14 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center">
            <svg
              ...
              strokeWidth={2.5} className="w-7 h-7 text-emerald-500"
            >
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Combobox
              ...
              value={watch('jenis') ?? ''}
              onChange={(val) =>
                setValue('jenis', val as FormData['jenis'], { shouldValidate: true })
              }
              placeholder="Pilih jenis izin"
              hasError={!!errors.jenis}
            />
            {errors.jenis && (
              <p className="text-xs text-red-500">{errors.jenis.message}</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              />
              {errors.tanggalMulai && (
                <p className="text-xs text-red-500">{errors.tanggalMulai.message}</p>
              )}
            </div>
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              ...
              {errors.tanggalSelesai && (
                <p className="text-xs text-red-500">{errors.tanggalSelesai.message}</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              rows={3}
              {...register('alasan')}
              placeholder="Tuliskan alasan izin kamu..."
            ...
            />
            {errors.alasan && (
              <p className="text-xs text-red-500">{errors.alasan.message}</p>
            )}
          </div>

          {/* File Bukti — opsional, kirim URL */}
          <div className="space-y-1.5">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              Link Bukti
            ...
            />
            {errors.fileBuktiUrl && (
              <p className="text-xs text-red-500">{errors.fileBuktiUrl.message}</p>
            )}
          </div>
...
# ─────────────────────────────────────────────────────────────────────────────
# 4. app/dashboard/absensi/scan/page.tsx
# ─────────────────────────────────────────────────────────────────────────────
...
// ── Inner Component (butuh Suspense karena useSearchParams) ───────────────────
...
function ScanContent() {
  ...
  // Auto-request GPS jika requireGps
  useEffect(() => {
  ...
  const handleAbsen = async () => {
    ...
    try {
      ...
      setErrorMsg(
        (status && SCAN_ERRORS[status]) ?? 'Terjadi kesalahan. Silakan coba lagi.',
      )
      setPageState('error')
    }
  }
  ...
  // Format tanggal: "2026-04-15" → "15/04/2026"
  const tanggalDisplay = sesi.tanggal.split('-').reverse().join('/')
  // Format expiresAt → jam lokal
      ...
      <div className="w-full max-w-sm space-y-4">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-sm text-gray-500">
            Pastikan data berikut sesuai sebelum absen
          </p>
        </div>
...
function GpsIndicator({ status }: { status: GpsStatus }) {
  ...
  return (
    <div
      className={
        'flex items-center gap-2 text-xs font-medium px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 ' +
        cls
      }
    >
...
// ── Page export — Suspense wajib karena useSearchParams ───────────────────────
...
if __name__ == '__main__':
    write_files()
    print("\n✅ Batch 3 selesai — 4 file digenerate.")
