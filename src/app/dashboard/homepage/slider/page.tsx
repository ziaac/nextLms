...
import type { Slider, CreateSliderDto } from '@/types/homepage.types'
...
function SortableSliderCard({
  ...
  const imgUrl = getPublicFileUrl(item.imageUrl)
    ...
    >
      ...
      {canWrite && (
        <button
          ...
          {...attributes}
          {...listeners}
          className="cursor-grab active:cursor-grabbing text-gray-300 dark:text-gray-600 hover:text-gray-500 touch-none shrink-0"
      ...
      {/* Thumbnail */}
      <div className="w-24 h-14 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 shrink-0">
        {imgUrl ? (
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.linkUrl && (
          <p className="text-xs text-blue-400 truncate mt-0.5">{item.linkUrl}</p>
        )}
      </div>

      {/* Badges */}
      <div className="flex items-center gap-2 shrink-0">
        <span className="text-xs font-mono text-gray-400">#{item.urutan}</span>
...
export default function SliderPage() {
  ...
  const { data: sliderList, isLoading } = useSliderList(false)
  const [ordered, setOrdered] = useState<Slider[]>([])
  useEffect(() => { setOrdered(sliderList ?? []) }, [sliderList])

  const [modalOpen,    setModalOpen]    = useState(false)
  const [editTarget,   setEditTarget]   = useState<Slider | null>(null)
  ...
  const reorderSlider = useReorderSlider()
  ...
  const handleSubmit = async () => {
    ...
    if (!form.imageUrl.trim()) { setFormError('Gambar slider wajib diupload'); return }
    setFormError(null)

    try {
      const dto: CreateSliderDto = {
      ...
      if (editTarget) {
        await updateSlider.mutateAsync({ id: editTarget.id, dto })
      } else {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    setOrdered(newOrder)
    await reorderSlider.mutateAsync(newOrder.map((m) => ({ id: m.id, urutan: m.urutan })))
  }
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center py-16 text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Belum ada slide</p>
          {canWrite && (
      ...
      <Modal
        ...
        title={editTarget ? 'Edit Slide' : 'Tambah Slide'}
        size="lg"
        footer={
          <>
      ...
      >
        <div className="px-6 py-5 space-y-4">
          ...
          <FileUpload
            label="Gambar Slide"
            hint="Rekomendasi: 1920×600px, JPG/PNG/WebP"
            accept=".jpg,.jpeg,.png,.webp"
            onUpload={(file) => uploadApi.homepageSlider(file).then((r) => r.url)}
            onSuccess={(url) => setForm((f) => ({ ...f, imageUrl: url }))}
            currentKey={form.imageUrl || null}
            compact
          />
          {form.imageUrl && (
            <div className="w-full h-32 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
              <img
                src={getPublicFileUrl(form.imageUrl)}
                alt="preview"
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder="Teks pendek di bawah judul..."
          ...
          <div className="space-y-1.5">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              Link URL <span className="text-xs text-gray-400">(opsional)</span>
            </label>
            <input
              type="text"
              value={form.linkUrl}
      ...
      <ConfirmModal
        ...
        title="Hapus Slide"
        description={`Hapus slide "${deleteTarget?.judul}"? Tindakan ini tidak dapat dibatalkan.`}
        confirmLabel="Hapus"
        variant="danger"
