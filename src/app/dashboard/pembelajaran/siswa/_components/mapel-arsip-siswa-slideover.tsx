...
function ArsipGroup({
  ...
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden">
      ...
      >
        <div>
          <p className="text-sm font-semibold text-gray-700 dark:text-gray-200">{kelas.tahunAjaran}</p>
          <p className="text-xs text-gray-400 dark:text-gray-500">{kelas.namaKelas} · {mapelList.length} mapel</p>
        </div>
        <ChevronDown
          size={16}
      ...
      {expanded && (
        <div className="p-3 space-y-2">
          ...
          {!isLoading && mapelList.map((mapel) => (
            <div
              key={mapel.id}
              className="rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/40 p-3 space-y-2"
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-sm font-medium text-gray-700 dark:text-gray-200 leading-tight">{mapel.namaMapel}</p>
                  <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">{mapel.kodeMapel} · {mapel.kategori}</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-xs text-gray-500 dark:text-gray-400">Kehadiran</p>
                  <p className="text-sm font-bold text-gray-700 dark:text-gray-200">{mapel.stats.absensiPercentage}%</p>
                </div>
              </div>
