...
import type { RosterGuruResponse, RosterGuruRosterItem } from '@/types/jadwal.types'
import { HARI_LABEL, collectTimeSlots, getActiveHari, getMapelPalette } from './MatrixUtils'

interface Props {
  roster: RosterGuruResponse
}

export function JadwalGuruMatrix({ roster }: Props) {
  ...
  if (timeSlots.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-gray-400 dark:text-gray-500">
        ...
        <p className="text-sm mt-1">Guru ini belum memiliki jadwal pada semester ini</p>
      </div>
    )
  }

  const rosterMap: Record<string, Record<string, RosterGuruRosterItem>> = {}
                      ...
                      <div className={`rounded-xl px-2.5 py-2 ${pal.bg}`}>
                        <div className="flex items-start gap-1.5">
                          ...
                          <div className="min-w-0">
                            ...
                            {item.kelas && (
                              <p className={`text-[10px] mt-0.5 truncate ${pal.sub}`}>{item.kelas.namaKelas}</p>
                            )}
                            {item.ruangan && (
